...
data<-readRDS("sodium_gcs_mediation_ 2020-07-28 .Rds")
data$gender<-ifelse(data$gender==1,"male","female")
data$hosp_mortality<-ifelse(data$hosp_mortality==1,"Died","Alive")
data$normal_gcs<-ifelse(data$normal_gcs==1,"Normal GCS","GCS<15")
...
#sodium_cat_5_df<-data%>%filter(sodium_cat==5)
...
## Table 1:
```{r,echo = F}
table1(~sodium_cat + age + gender + AP_grp +ethnicity + AP_grp + mechvent_day01 + unittype + as.factor(charlson_chf1) + as.factor(charlson_liver1)|normal_gcs+hosp_mortality,data = data)
```
...
#treat is the independend variable
mediation_results_cat8 <- mediate(fit_mediator, fit_dv, treat="sodium_cat", mediator="gcs_baseline",sims = 2000)
