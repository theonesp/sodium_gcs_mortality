...
```{r}
total_effect<-NULL
fit_totaleffect<-glm(as.factor(hosp_mortality)~sodium_cat+age+AP_grp+gender+ethnicity+unittype+charlson_liver1 +charlson_chf1+mechvent_day01

                   , sodium_cat_1_df
,family = binomial)
summ<-summary(fit_totaleffect)
total_effect<-rbind(total_effect,data.frame(cat = 1,estimated = summ$coefficients[2],se = summ$coefficients[2+length(summ$coefficients)/4]))
...
mediation_results_cat1 <- mediate(fit_mediator, fit_dv, treat="sodium_cat", mediator="gcs_baseline",sims = 100)
...
,family = binomial)
summ<-summary(fit_totaleffect)
total_effect<-rbind(total_effect,data.frame(cat = 2,estimated = summ$coefficients[2],se = summ$coefficients[2+length(summ$coefficients)/4]))
...
summary(fit_dv)
#treat is the independend variable
mediation_results_cat2 <- mediate(fit_mediator, fit_dv, treat="sodium_cat", mediator="gcs_baseline",sims = 1000)
...
,family = binomial)
summ<-summary(fit_totaleffect)
total_effect<-rbind(total_effect,data.frame(cat = 3,estimated = summ$coefficients[2],se = summ$coefficients[2+length(summ$coefficients)/4]))
...
summary(fit_dv)
#treat is the independend variable
mediation_results_cat3 <- mediate(fit_mediator, fit_dv, treat="sodium_cat", mediator="gcs_baseline",sims = 1000)
...
,family = binomial)
summ<-summary(fit_totaleffect)
total_effect<-rbind(total_effect,data.frame(cat = 4,estimated = summ$coefficients[2],se = summ$coefficients[2+length(summ$coefficients)/4]))
fit_mediator<-lm(gcs_baseline ~ sodium_cat+age+AP_grp+gender+ethnicity+unittype+charlson_liver1 +charlson_chf1+mechvent_day01,
...
summary(fit_dv)
#treat is the independend variable
mediation_results_cat4 <- mediate(fit_mediator, fit_dv, treat="sodium_cat", mediator="gcs_baseline",sims = 1000)
...
,family = binomial)
summ<-summary(fit_totaleffect)
total_effect<-rbind(total_effect,data.frame(cat = 6,estimated = summ$coefficients[2],se = summ$coefficients[2+length(summ$coefficients)/4]))
...
summary(fit_dv)
#treat is the independend variable
mediation_results_cat6 <- mediate(fit_mediator, fit_dv, treat="sodium_cat", mediator="gcs_baseline",sims = 1000)
...
,family = binomial)
summ<-summary(fit_totaleffect)
total_effect<-rbind(total_effect,data.frame(cat = 7,estimated = summ$coefficients[2],se = summ$coefficients[2+length(summ$coefficients)/4]))
...
summary(fit_dv)
#treat is the independend variable
mediation_results_cat7 <- mediate(fit_mediator, fit_dv, treat="sodium_cat", mediator="gcs_baseline",sims = 1000)
...
,family = binomial)
summ<-summary(fit_totaleffect)
total_effect<-rbind(total_effect,data.frame(cat = 8,estimated = summ$coefficients[2],se = summ$coefficients[2+length(summ$coefficients)/4]))
...
summary(fit_dv)
#treat is the independend variable
mediation_results_cat8 <- mediate(fit_mediator, fit_dv, treat="sodium_cat", mediator="gcs_baseline",sims = 1000)
...
```{r}
total_effect<-rbind(total_effect,data.frame(cat = 5,estimated = 0,se = 0))
...
p<-ggplot()+
    geom_area(data=mediation_results_propmediated,aes(x=cat,y=Estimate,fill=results))+
    xlab('Baseline Sodium Category')+
    ylab('%')+
  ...
  geom_point(data = total_effect,aes(x=cat,y=or-1,colour="red"))+
  geom_line(data = total_effect,aes(x=cat,y=or-1,colour="red"))+
  geom_errorbar(data=total_effect
                  ,aes(x=cat,ymin=or.lb-1,ymax=or.ub-1)
                  ,alpha=0.3
                  ,inherit.aes=FALSE
                  ,width=0.2)+
  scale_y_continuous(
    ...
    name = "Proption Mediated",
    
    # Add a second axis and specify its features
    sec.axis = sec_axis(~(.+1), name="Odds ratio")
  ) 
