...
ggplot(data=mediation_results_propmediated,aes(x=cat,y=Estimate*100))+
  ...
  # scale_fill_manual(labels = c("Proportion Mediated"))+
  # labs(colour='Mortality Risk', fill='Proportion Mediated') +
  theme(legend.position="bottom")+
  theme_minimal()+
  scale_x_continuous(breaks = c(1,2,3,4,5,6,7,8),labels = c("<120","120- <125","125- <130","130- <135","135- <140\n(Reference)","140- <145","145- <150",">150")) +
  geom_point(data = total_effect,aes(x=cat,y=(or-1)*100),colour="#34495e")+
  geom_line(data = total_effect,aes(x=cat,y=(or-1)*100),colour="#34495e")+
  geom_ribbon(data=total_effect
...
# Sensitivity analysis plot grid

Ref.: plot(sens.out2, sens.par = "rho", main = "Sensitivity Analysis", ylim = c(-0.05, 0.05)) plot(sens.out8, sens.par = "rho", main = "Sensitivity Analysis", ylim = c(-0.05, 0.05))

```{r}
plots <- list()

# Use a loop to generate and store the plots
for(i in 1:8){
  if(i == 5) next # Skip plot number 5
  
  # Generate a plot
  sens.out <- as.list(get(paste("sens.out", i, sep = "")))
  
}  
  
  
  # Add a title to the plot
  p <- plot(sens.out, sens.par = "rho", main = "Sensitivity Analysis", ylim = c(-0.05, 0.05))
  
  # Store the plot in the list
  plots[[i]] <- p
}

# Plot the plots in a 3x3 grid
grid.arrange(grobs = plots, nrow = 3)



plot(sens.out1, sens.par = "rho", main = "Sensitivity Analysis Cat.1", ylim = c(-0.05, 0.05))
```
