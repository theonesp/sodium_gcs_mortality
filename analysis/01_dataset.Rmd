...
project_id <- "resuscitation-chf-esrd"
...
rass_prep <- RASS%>%group_by(patientunitstayid)%>%
  arrange(desc(nursingchartoffset))%>%
  filter(row_number()==1)
rass_prep$rass_score <- ifelse(rass_prep$nursingchartvalue < 0,",<0","other")
rass_prep = rass_prep%>%select(patientunitstayid,rass_score)
...
selected_df$adm_source = ifelse(selected_df$hospitaladmitsource%in%c("Emergency Department"), "ED",ifelse(adm_source$hospitaladmitsource%in%c("Other Hospital"),"Other Hospital","Floor/Other"))
selected_df$rass_score[is.na(selected_df$rass_score)] = "other" ## Problem is only 6k out of 200k has RASS
...
selected_df$adm_source<-as.factor(selected_df$adm_source)
...
print('Patients with incomplete data and elective surgery that are excluded')
selected_df<-selected_df[complete.cases(selected_df),]
selected_df<-selected_df%>%filter(electivesurgery == 0)
b<-nrow(selected_df)
a-b
...
flow_exclusions(
  ...
  excl_labels = c("Patients with incomplete data and elective surgery that are excluded"),
...
#exclude missing GCS sodium at baseline. Update here since labs and measurements weren't imputated before.
print('Patients with incomplete data that are excluded')
#selected_df<-selected_df[complete.cases(selected_df),]
selected_df<-selected_df%>%filter(!is.na(gcs_baseline)&!is.na(sodium1)&!is.na(hosp_mortality))
b<-nrow(selected_df)
a-b
...
flow_exclusions(
  ...
  total_label = "Initial Cohort",
  incl_labels = c( "Final Number of patients"),
  excl_labels = c("Patients with missing sodium, gcs and hospital discharge stauts are excluded"),
  percent_of_total = TRUE
)
```
...
selected_df_imputed[,c("eyes","motor","verbal","meds","urine","wbc","temperature","heartrate","meanbp","ph","hematocrit","creatinine","albumin","pao2","pco2","bun","glucose","bilirubin","fio2"                     ,"admitsource","ejectfx","dischargelocation","amilocation","day1meds","day1verbal","day1motor","day1eyes","day1pao2","day1fio2","MAP","respiratoryrate")]<-apply(selected_df_imputed[,c("eyes","motor","verbal","meds","urine","wbc","temperature","heartrate","meanbp","ph","hematocrit","creatinine","albumin","pao2","pco2","bun","glucose","bilirubin","fio2"                     ,"admitsource","ejectfx","dischargelocation","amilocation","day1meds","day1verbal","day1motor","day1eyes","day1pao2","day1fio2","MAP","respiratoryrate")],2, function(x) median_imp(x) )

selected_df_imputed[,c('cirrhosis','aids','leukemia','immunosuppression','charlson_chf1','charlson_liver1')]<-apply(selected_df_imputed[,c('cirrhosis','aids','leukemia','immunosuppression','charlson_chf1','charlson_liver1')],2, function(x) na_replace_zero(x) )
...
>>>>>>> update_data_and_analysis:analysis/01_dataset.Rmd
