...
sedation_score <- run_query( getSQL("sql/sedation_score.sql" ))
```
...
# Exclude missing sedation_score
print('Patients with sedation score equal to "Exclude" that are excluded:')
selected_df %<>%
  filter(sedation_score != 'Exclude' | is.na(sedation_score))
c <- nrow(selected_df)
b - c
...
flow_exclusions(
  incl_counts = c(a, b, c),  # Utilizar estos valores
  total_label = "Total eICU database",
  incl_labels  = c("Patients with available sodium, gcs and hospital discharge status", "Final Cohort"),
  excl_labels = c("Patients with missing sodium, gcs and hospital discharge status are excluded", "Patients with sedation score equal to Exclude that are excluded:")
)
```
...
selected_df %<>%
  mutate_at(
    vars(c('cirrhosis', 'aids', 'leukemia', 'immunosuppression', 'charlson_chf1', 'charlson_liver1',
           'gender', 'ethnicity', 'AP_grp', 'unittype', 'hospitaladmitsource', 'mechvent_day01', 'hosp_mortality','adm_source','sedation_score')),
    as.factor
  )
```
...
selected_df_imputed <- selected_df_imputed %>%
  mutate(
    ...
    sedation_score = if_else(is.na(selected_df_imputed$sedation_score),'Unknown',selected_df_imputed$sedation_score
    )
  )
