...
```
# Figure 3B: Not notsedated by Med Subanalysis
...
supa <- ggplot(data = mediation_results_notsedated_propmediated, aes(x = cat, y = Estimate * 100)) +
  geom_bar(stat = 'identity', fill = "#27ae60") +
  xlab('Baseline Sodium Category') +
  ylab('%') +
...
supb <- ggplot(data = mediation_results_sedated_propmediated, aes(x = cat, y = Estimate * 100)) +
...
combined_plot <- plot_grid(supa, supb, labels = c("", ""), ncol = 2)
...
```{r}
tiff("figures/sup_figure.tiff", width = 16*250, height = 9*200, res = 300)
print(combined_plot)
dev.off()
```
